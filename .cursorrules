...
- **Strict Version Bump Law.** Every single **source code** change (app logic, UI, runtime configs) MUST increment the version code/number, update `version.py`/`version.properties`/`package.json`, and document changes in `Nucleus — Что сделано, планы и журнал.md` and today's daily note before reporting back. **Code changes → version bump + ADB deploy + EXE/installer rebuild + Git commit & push.** Markdown/rule-only changes (`.md`, `AGENTS.md`, `GEMINI.md`, `CLAUDE.md`, `.cursorrules`, `README.md`) do NOT trigger version bumps or binary rebuilds — update only the relevant MD files and stop there.
...
- **Основная проектная триада (Всегда в актуальном состоянии):**
  1. `Nucleus — Описание проекта (архитектура).md` — архитектура, стек, подсистемы.
